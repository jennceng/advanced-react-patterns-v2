...
class Toggle extends React.Component {
  ...
  // you don't want to use a reference to this.state because you want this.setState to be able to reference the current state and avoid set state batching
  internalSetState = (updater, callback) => {
    if(typeof updater === "function") {
      const updatedState = updater(this.state)
      const userChanges = this.props.stateReducer(this.state, updatedState)
      if(Object.keys(userChanges).length > 0) {
        // you dont want to pass in updater here, it'll automatically invoke it with no checking unlike the return from the stateReducer which has some conditional
        return this.setState(userChanges, () => callback())
      } else {
        return null;
      }
    }
  ...
  //     return [changes]
  //       .map(change => typeof change === "function" ? change(this.state) : change)
  //       .map(change => this.props.stateReducer(this.state, change) || {})
  //       .map(change => Object.keys(change).length > 0 ? change : null)[0]
  //     }, callback)
  //   }
